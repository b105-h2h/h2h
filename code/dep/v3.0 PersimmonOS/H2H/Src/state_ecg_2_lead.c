...
void behaviour_ecg_2_lead(state_ptr state)
{
  ...
  menu_ecg.items[4].item.area.is_active = GUI_INACTIVE;
  menu_ecg.items[4].item.area.text_color = (color_t) COLOR_BLUE;
  menu_ecg.items[4].item.area.bg_color = (color_t) COLOR_WHITE;
