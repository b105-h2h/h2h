...
/* State behaviour */
void behaviour_settings_configtabs_about(state_ptr state)
{
  ...
  menu_settings_configtabs.items[5].item.area.is_active = GUI_INACTIVE;
  menu_settings_configtabs.items[5].item.area.text_color = (color_t) COLOR_BLUE;
  menu_settings_configtabs.items[5].item.area.bg_color = (color_t) COLOR_WHITE;
...
void entry_to_settings_configtabs_about(state_ptr state)
{
  /* Set state name */
  strcpy(state->name, "settings_configtabs_about");
  ...
  behaviour_settings_configtabs_about(state);
