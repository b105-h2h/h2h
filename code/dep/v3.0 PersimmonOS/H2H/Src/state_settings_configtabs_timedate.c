...
void behaviour_settings_configtabs_timedate(state_ptr state)
{
  ...
  menu_settings_configtabs.items[2].item.area.is_active = GUI_INACTIVE;
  menu_settings_configtabs.items[2].item.area.text_color = (color_t) COLOR_BLUE;
  menu_settings_configtabs.items[2].item.area.bg_color = (color_t) COLOR_WHITE;
...
//  /* Set menu */
//  osMailPut(queue_input_menuHandle, (void *) &menu_settings_configtabs_timedate);
//
//  /* Display menu */
//  uint32_t i;
//  for (i = 0; i < menu_settings_configtabs_timedate.item_num; i++)
//  {
//    osMailPut(queue_lcdHandle, (void *) &menu_settings_configtabs_timedate.items[i]);
//  }
}
